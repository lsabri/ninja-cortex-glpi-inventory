...
async function writeToSheet(rows, sheetName) {
  try {
    ...
    const headers = sheetName === SHEET_NAME_NINJA
      ? ['Nom', 'OS', 'OS Release', 'Agent Version', 'Last Contact']
      : ['Nom', 'OS', 'OS Release', 'Agent Version', 'Last Seen'];

    const dateRow = [`Dernière mise à jour : ${new Date().toLocaleString('fr-FR')}`];

    // Supprime colonne ID
    const rowsWithoutID = rows.map(r => r.slice(1));
...
async function getNinjaDevices() {
  ...
  try {
    ...
    for (const d of devices) {
      ...
      // ----------------- OS exact comme dans l'API -----------------
      const osName = d.os?.name || "OS inconnu";
      const osRelease = d.os?.releaseId || "N/A";
      ...
      try {
        ...
        if(customResp.data.versionAgentNinjaone) agentVersion = customResp.data.versionAgentNinjaone;
      } catch {
        writeLog(`⚠️ Impossible de récupérer versionAgentNinjaone pour ${name}`);
      }

      // Stocke avec ID en première colonne mais on supprime à l'écriture
